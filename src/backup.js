...
function getPeriodoDates(periodo) {
  ...
  const fim = new Date(); // sempre agora
  let inicio = new Date();
...
app.get("/api/indicadores/taxa-resolucao", async (req, res) => {
  try {
    ...
    if (mapa[periodo] !== undefined) {
      inicio = new Date(agora.getTime() - mapa[periodo] * 86400000);
    } else if (periodo === "6m") {
      inicio = new Date(agora);
      inicio.setMonth(inicio.getMonth() - 6);
    } else if (periodo === "1y") {
      ...
      const [[minDate]] = await db.query(
        `SELECT MIN(created_at) AS primeira FROM jp_conectada.solicitations`
      );
      inicio = minDate?.primeira || new Date("2000-01-01");
    }
    ...
    const sql = `
      SELECT
        SUM(CASE WHEN created_at BETWEEN ? AND ? THEN 1 ELSE 0 END) AS iniciadas,

        SUM(CASE WHEN status IN (1,4)
                 AND updated_at BETWEEN ? AND ?
             THEN 1 ELSE 0 END) AS resolvidas,

        AVG(
          CASE WHEN status IN (1,4)
                   AND updated_at BETWEEN ? AND ?
    ...
    const iniciadas = Number(row.iniciadas || 0);
    const resolvidas = Number(row.resolvidas || 0);
    const horasMedio = Number(row.horas_medio || 0);

    const dias = Math.floor(horasMedio / 24);
    const horas = Math.round(horasMedio % 24);

    const taxaResolucao =
      iniciadas > 0 ? (resolvidas / iniciadas) * 100 : 0;

    // --------------------------------------------------------------
    // 3) TAXA DE RESPOSTAS (não concluídas + respondidas)
    // --------------------------------------------------------------

    const sqlTaxaRespostas = `
      SELECT
    ...
    const [[rowResp]] = await db.query(sqlTaxaRespostas, [inicio, fim]);

    const iniciadasResp = Number(rowResp.iniciadas || 0);
    const respondidas = Number(rowResp.respondidas || 0);

    const taxaRespostas =
      iniciadasResp > 0 ? (respondidas / iniciadasResp) * 100 : 0;

    // --------------------------------------------------
    // 4) Resposta final COMPLETA
    // --------------------------------------------------
    res.json({
      periodo,
      inicio,
      ...
      iniciadas,
      resolvidas,
      taxa_resolucao: Number(taxaResolucao.toFixed(1)),
      tempo_medio: { dias, horas },

      // respostas
      taxa_respostas: {
        percentual: Number(taxaRespostas.toFixed(1)),
        iniciadas: iniciadasResp,
        respondidas
      }
    });
    ...
    res.status(500).json({ erro: "Falha ao carregar indicador" });
  }
});
