// ============================================================
// SETORES.TSX — COMPLETO COM ORDENAÇÃO, POPUP E TOOLTIP
// ============================================================
...
const nomeSetorUsuarios = topUsuario
  ...
  // clique normal → ordena por uma coluna
  // Shift + clique → adiciona/remaneja multi-coluna
  ...
  useEffect(() => {
    async function loadAll() {
      try {
        ...
        const efTop = (eficienciaList || [])
          .filter((e) => Number(e.total_solicitacoes) > 0)
          .sort(
            (a, b) =>
        ...
        setRankEficiencia(efTop);
        setGraficoEficiencia(
          efTop.map((e) => ({
            setor: e.setor,
            eficiencia: Number(e.eficiencia_percentual || 0)
          }))
        );
        ...
        const qTop = [...(qualidadeList || [])]
          .sort(
            (a, b) =>
              Number(b.nota_media ?? 0) -
              Number(a.nota_media ?? 0)
          )
          .slice(0, 5);
  ...
  ============================================================ */
  useEffect(() => {
    if (!chartEficienciaRef.current || graficoEficiencia.length === 0) return;
    ...
    chartEficienciaInst.current = new Chart(chartEficienciaRef.current, {
      type: "bar",
      data: {
        labels: graficoEficiencia.map((e) => e.setor),
        datasets: [
          {
            label: "Eficiência (%)",
            data: graficoEficiencia.map((e) => e.eficiencia),
            backgroundColor: gradient,
            borderRadius: 8,
            barThickness: 22
          }
